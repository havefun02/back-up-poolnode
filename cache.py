...
class UserConnectionCache:
    ...
    def add_connection(self, username, socket,job_id,user_id,target,block,hashrate):
        expire_time = datetime.now() + timedelta(hours=12)
        data = {'socket': socket, 'expire': expire_time,"job_id":job_id,"user_id":user_id,"target":target,"block":block,"hashrate":hashrate}
