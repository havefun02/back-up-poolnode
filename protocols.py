...
def open_handler(data,socket):
    ...
    if miner.password==auth.password and miner.username==auth.username:
        ...
        if (job is None):
            ...
            user_cache.add_connection(miner.username,socket,None,miner.user_id,miner.target,None,(miner.hashrate))
        else:
            user_cache.add_connection(miner.username,socket,job.job_id,miner.user_id,miner.target,ast.literal_eval(job.block),(miner.hashrate))
...
def submit_handler(data):
    ...
    if status!=0:
        ...
        hash_rate=int(str(miner["block"]["hashrate"]))

        miner_hashrate=int((miner["block"]["hashrate"]+hash_rate)/2)
        # logger.log_info(miner_hashrate)
        ...
        database.add_data(t)
        database.custom_query(f"update miners set hashrate={miner_hashrate} where username='{username}';")
...
def update_target(start,end):
    ...
    try:
        ...
        for miner in miners:
            new_target=0
            hashrate=int(miner[2])
            print(hashrate)
            d=(hashrate*2)/pow(2,8)
            int_target=(int(str(DEFAUTL_TARGET),16))
            new_target=int(int_target/int(d))
