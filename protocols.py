...
def open_handler(data,socket):
    ...
    if miner.password==auth.password and miner.username==auth.username:
        job=database.find_one(JobRecord,order_by_column="job_id",descending=True,id_user=miner.id)
        if (job is None):
...
def request_job_handler(data):
    ...
    if (current_job is None):
        ...
        job=create_jobs(result_string,job_id,miner["target"],miner["block"])
    # if (job is None):
    job_record=JobRecord(id_user=miner["user_id"],job_id=job_id,block=str(job))
    miner["job_id"]=job_id
    miner["block"]=job
...
def int_to_32byte_hex(num):
    hex_string = hex(num)[2:]  # Convert integer to hexadecimal string
